...
const dayOne = [
  ["9h00 - 11h00", "Hội thảo chuyên đề phát triển thị trường tiêu dùng trong nước đối với ngành hàng hóa mỹ phẩm."],
...
const dayTwo = [
  ...
  ["9h00 - 12h00", "Hội thảo Marketing phiên 1: kiến tạo hệ thống tăng trưởng doanh thu trong kỷ nguyên AI."],
  ["13h30 - 15h00", "Hội thảo Marketing phiên 2: xây dựng bộ máy Marketing hiệu suất cao."],
  ["15h30 - 17h30", "Beauty Summit Awards."],
  ["17h30", "Bế mạc Beauty Summit 2026."]
];
...
const glowButtonClass = "relative inline-flex min-h-11 min-w-[142px] items-center justify-between gap-3 overflow-hidden rounded-full bg-[linear-gradient(135deg,#2b3064_0%,#8e44ad_50%,#e91e63_100%)] py-1.5 pl-4 pr-1.5 text-sm font-extrabold text-white shadow-[0_12px_24px_rgba(92,42,156,0.32),inset_0_0_0_1px_rgba(255,255,255,0.3)] before:absolute before:bottom-[-40%] before:left-[-70px] before:top-[-40%] before:w-[54px] before:rotate-[18deg] before:animate-[lp-cta-shine_2.2s_ease-in-out_infinite] before:bg-[linear-gradient(90deg,transparent,rgba(255,255,255,0.8),transparent)] sm:min-h-12 sm:min-w-[164px] sm:pl-5 sm:text-sm lg:min-h-[54px] lg:min-w-[188px] lg:pl-6 lg:text-base";
    ...
    <main className="min-h-screen overflow-hidden bg-[#fff] text-[#fff8e8]">
      ...
      <section className={`${sectionShell} bg-[linear-gradient(180deg,#ffffff,#fff6fb_52%,#f5efff)]`}>
        <div className="mx-auto mb-9 max-w-[980px] text-center">
          ...
          <h2 className="mt-4 text-[30px] font-black uppercase leading-[1.08] text-[#211D51] sm:text-[44px] lg:text-[56px]">
            Quà tặng & trải nghiệm độc quyền
          </h2>
        </div>
      ...
      <section className={`${sectionShell} bg-[#fff]`}>
        <div className={sectionHead}>
          <p className="text-sm font-extrabold uppercase text-[#a36f1d]">Hướng dẫn mua vé</p>
          <h2 className="mt-2 text-[28px] font-black uppercase leading-[1.08] text-[#222] sm:text-[40px] lg:text-[52px]">Quy trình tham dự</h2>
        </div>
        <div className="mx-auto grid w-full max-w-[1080px] gap-4 lg:grid-cols-4">
          {steps.map(([number, title, text]) => (
            <article className="min-h-[230px] border border-[#333] bg-[#f1f1f1] rounded-[12px] p-6 text-[#fff4dc]" key={number}>
              <strong className="mb-5 grid h-12 w-12 place-items-center bg-[#ccc] rounded-[8px] text-2xl font-black text-[#4b0712]">{number}</strong>
              <h3 className="text-lg font-bold text-[#222]">{title}</h3>
              <p className="mt-2.5 text-sm leading-6 text-[#333]">{text}</p>
            </article>
          ))}
        </div>
        <ActionButtons className="mt-8" onBuy={openTicketModal} onViewBenefits={scrollToBenefits} />
      ...
      <section className="bg-[linear-gradient(110deg,#ef4266_0%,#9520a8_100%)] px-4 py-12 sm:px-6 lg:px-[72px] lg:py-14 lg:pb-24">
        ...
        <div className="relative mx-auto mt-9 max-w-[1480px] rounded-2xl bg-[#ffe4ee] px-6 py-7 text-[#33333b] sm:px-9 lg:px-12 lg:py-9">
          <ul className="space-y-5 text-sm font-bold leading-7 sm:text-base">
            ...
            <li className="flex gap-3">
              <ChevronRight className="mt-1 h-5 w-5 shrink-0 text-[#ff5b72]" strokeWidth={3} />
              <p>
                BTC chỉ phân phối vé duy nhất thông qua website chính thức https://beautysummit.vn/dang-ky-mua-ve. Quý khách vui lòng cảnh giác với những dịch vụ đặt vé không có nguồn gốc rõ ràng và các bên trung gian. BTC sẽ không giải quyết mọi trường hợp tranh chấp vé.
              </p>
            </li>
            <li className="flex gap-3">
              <ChevronRight className="mt-1 h-5 w-5 shrink-0 text-[#ff5b72]" strokeWidth={3} />
...
function TicketOrderModal({
  ...
  return (
    <div
      className={[
        "fixed inset-0 z-[80] grid place-items-center overflow-y-auto bg-[#17051f99] px-3 py-6 backdrop-blur-md",
        isClosing ? "animate-[lp-modal-backdrop-out_0.22s_ease_both]" : "animate-[lp-modal-backdrop-in_0.24s_ease_both]"
      ].join(" ")}
    >
      <section
        className={[
          "relative w-full max-w-[720px] rounded-[18px] border border-[#dfcdfa] bg-white p-5 text-[#05083c] shadow-[0_24px_70px_rgba(42,15,75,0.24)] sm:p-7",
        ...
        {step === "tickets" ? (
          <>
            <div className="mb-6 flex items-center gap-4 pr-10">
              ...
              <span className="h-px flex-1 bg-[#eadcff]" />
            </div>
            ...
            <button
              className="mt-5 min-h-[54px] w-full rounded-full bg-[linear-gradient(90deg,#e8a2d0,#c0a0f1)] text-sm font-black uppercase tracking-[0.14em] text-white shadow-[0_14px_28px_rgba(141,84,210,0.26)] disabled:cursor-not-allowed disabled:opacity-55"
          ...
          <>
            <div className="mb-6 flex items-center gap-4 pr-10">
              <button
                className="grid h-9 w-9 place-items-center rounded-full border border-[#eadcff] bg-white text-lg font-black text-[#1d1f5f]"
                type="button"
                onClick={onBack}
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              <label className="grid gap-2 text-sm font-bold text-[#07114a] sm:col-span-2">
                Họ và tên *
                <input
                  className="min-h-12 rounded-xl border border-[#dfcdfa] px-4 font-medium outline-none focus:border-[#9c64df] focus:ring-4 focus:ring-[#b16bea24]"
              ...
              <label className="grid gap-2 text-sm font-bold text-[#07114a]">
                Số điện thoại *
                <input
                  className="min-h-12 rounded-xl border border-[#dfcdfa] px-4 font-medium outline-none focus:border-[#9c64df] focus:ring-4 focus:ring-[#b16bea24]"
              ...
              <label className="grid gap-2 text-sm font-bold text-[#07114a] sm:col-span-2">
                Giới tính *
                <select
                  className="min-h-12 rounded-xl border border-[#dfcdfa] bg-white px-4 font-medium outline-none focus:border-[#9c64df] focus:ring-4 focus:ring-[#b16bea24]"
                  value={form.gender}
                  onChange={(event) =>
...
function TicketQuantityRow({
  ...
  return (
    <article className="grid gap-4 rounded-2xl border border-[#dfcdfa] bg-white p-4 shadow-[0_10px_24px_rgba(88,54,130,0.06)] sm:grid-cols-[1fr_auto] sm:items-center">
      <div className="flex min-w-0 items-center gap-4">
        {ticket.img ? (
          <img className="h-[58px] w-[168px] shrink-0 object-contain" src={ticket.img} alt={ticket.name} />
        ) : (
        ...
        <div className="min-w-0">
          <h3 className="truncate text-xl font-black uppercase text-[#b1132a]">{ticket.name}</h3>
          <p className="mt-1 text-xl font-black text-[#f43255]">{formatCurrency(currentPrice)}</p>
        </div>
      </div>

      <div className="grid h-12 grid-cols-3 overflow-hidden rounded-xl border border-[#dfcdfa] bg-white justify-self-start sm:justify-self-end">
        <button
          className="grid w-12 place-items-center text-xl font-black text-[#80688d]"
          type="button"
        ...
        <input
          className="w-11 border-x border-[#dfcdfa] text-center text-sm font-black text-[#07114a] outline-none"
          readOnly
          type="number"
          value={quantity}
        />
...
function OrderSummaryBox({ summary }: { summary: ReturnType<typeof calculateCartSummary> }) {
  return (
    <div className="mt-7 rounded-2xl border border-[#dfcdfa] bg-[#fffdfd] p-5">
      <div className="flex items-center justify-between text-sm text-[#5f5170]">
        <span>Tạm tính</span>
        <span>{formatCurrency(summary.subtotal)}</span>
      </div>
      <div className="mt-4 flex items-center justify-between border-t border-[#eadcff] pt-4">
        <span className="text-xl font-black text-[#07114a]">Tổng thanh toán</span>
        <span className="text-2xl font-black text-[#bb59e4]">{formatCurrency(summary.total)}</span>
      </div>
    </div>
  );
}
